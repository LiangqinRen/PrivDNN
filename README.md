...
The *select* function has five modes. The first four are corresponded to 4 algorithms. PrivDNN will save the selected neurons to the file *PrivDNN/saved_models/[dataset]/selected_neurons.json*. Mode 5 will test all possible selections as the ground truth for MNIST and EMNIST, which takes about three days in our environment. Every algorithm has some specific approaches, such as PFEC and FPGM, and we execute only one approach during the experiments to avoid the affection of cache, etc. We don't implement random selection here because we have the ground truth for all possible selections of MNIST and EMNIST. We will explain this later.
