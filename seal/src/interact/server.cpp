...
bool is_file_complete(char *dataset) {
  if (string(dataset) == string("MNIST")) {
    ...
    const array<string, 4> file_names{
        string("_conv1_weight"), string("_conv1_bias"), string("_conv2_weight"),
        string("_conv2_bias")};
...
void save_trained_data(char *dataset, double *trained_data) {
  if (string(dataset) == string("MNIST")) {
    ...
    const string conv1_weight_path =
        MNIST_path + files_prefix + string("_conv1_weight");
      ...
      for (size_t j = 0; j < conv1_weight_size; ++j) {
        if (is_neuron_encrypted(encryped_neurons, 1, i)) {
          ...
          conv1_weight_outstream.write(
              reinterpret_cast<const char *>(&temp_number),
              sizeof(temp_number));
        }
    ...
    const string conv1_bias_path =
        MNIST_path + files_prefix + string("_conv1_bias");
    ...
    for (size_t i = 0; i < conv1_bias_shape; ++i) {
      if (is_neuron_encrypted(encryped_neurons, 1, i)) {
        ...
        double temp_number = trained_data[trained_data_index];
        conv1_bias_outstream.write(reinterpret_cast<const char *>(&temp_number),
                                   sizeof(temp_number));
      }
    ...
    const string conv2_weight_path =
        MNIST_path + files_prefix + string("_conv2_weight");
    ...
    const string conv2_bias_path =
        MNIST_path + files_prefix + string("_conv2_bias");
    ...
    for (size_t i = 0; i < conv2_bias_shape; ++i) {
      if (is_neuron_encrypted(encryped_neurons, 2, i)) {
        ...
        double temp_number = trained_data[trained_data_index];
        conv2_bias_outstream.write(reinterpret_cast<const char *>(&temp_number),
                                   sizeof(temp_number));
      }
...
void thread_worker_decrypt_result(const vector<Ciphertext> &ciphers,
                                  vector<double> &numbers, int begining,
                                  int ending) {
  auto parms = read_parms();
  auto secret_key = read_secret_key();
  SEALContext context(parms);
...
void thread_decrypt_result(const vector<Ciphertext> &ciphers,
                           vector<double> &numbers) {
  const unsigned processor_count = std::thread::hardware_concurrency();
  vector<int> threads_task_count(processor_count,
                                 ciphers.size() / processor_count);
  ...
  for (size_t i = 0; i < threads.size(); ++i) {
    threads[i] =
        thread(thread_worker_decrypt_result, cref(ciphers), ref(numbers),
               beginnig, beginnig + threads_task_count[i]);
...
vector<double> read_result(const string &dataset, SEALPACK &seal,
                           array<size_t, 4> output_shape, size_t output_size,
                           mode work_mode) {
    ...
    for (size_t i = 0; i < output_size / output_shape[0];) {
      if (is_neuron_encrypted(encrypted_neurons, 2, i / block_size) &&
        ...
        for (size_t j = 0; j < block_size; ++i, ++j) {
          double value;
          for (size_t k = 0; k < batch_size; ++k) {
            result_stream.read(reinterpret_cast<char *>(&value),
                               sizeof(double));
...
double *get_result(char *dataset, int batch_size, mode work_mode = separate_) {
  ...
  auto final_result =
      read_result(string(dataset), seal, output_shape, output_size, work_mode);
