...
size_t get_index(const array<size_t, 4> &shape,
                 const array<size_t, 4> &indexes) {
...
vector<variant<double, Ciphertext>> read_conv_weight(string dataset,
                                                     SEALPACK &seal, int round,
  ...
  const string weight_path =
      move(DATA_PATH + string(dataset) + string("/") + files_prefix +
           string("_conv") + to_string(round) + string("_weight"));
  ...
  for (size_t i = 0; i < weight_shape[0]; ++i) {
    if (is_neuron_encrypted(encrypted_neurons, round, i)) {
      ...
      for (size_t j = 0; j < block_size; ++j) {
        ...
        trained_data_input_stream.read(reinterpret_cast<char *>(&weight_value),
                                       sizeof(weight_value));
...
vector<variant<double, Ciphertext>> read_conv_bias(string dataset,
                                                   SEALPACK &seal, int round,
                                                   json encrypted_neurons) {
  MNIST_Shape shapes;
  hash<json> encrypted_list_hash;
  auto files_prefix = to_string(encrypted_list_hash(encrypted_neurons));
  const string bias_path =
      move(DATA_PATH + string(dataset) + string("/") + files_prefix +
           string("_conv") + to_string(round) + string("_bias"));
  ...
  for (size_t i = 0; i < bias.size(); ++i) {
    if (is_neuron_encrypted(encrypted_neurons, round, i)) {
      ...
      double bias_value;
      MNIST_bias_instream.read(reinterpret_cast<char *>(&bias_value),
                               sizeof(bias_value));
      bias[i] = bias_value;
    }
  }
...
void thread_conv_worker(
    SEALPACK &seal, const vector<array<size_t, 4>> &output_indexes,
    const vector<variant<vector<double>, Ciphertext>> &input,
    const vector<variant<double, Ciphertext>> &weight, Ciphertext bias,
    vector<variant<vector<double>, Ciphertext>> &result, MNIST_Shape shapes,
    size_t round, int begining, int ending) {
  ...
  for (int i = begining; i < ending; ++i) {
    ...
    if (round == 1) {
      for (size_t a = 0; a < shapes.kernal_sizes[1]; ++a) {
        for (size_t b = 0; b < shapes.kernal_sizes[1]; ++b) {
          ...
          auto weight_cipher = get<Ciphertext>(weight[get_index(
              shapes.conv_weight[1], {output_indexes[i][1], 0, a, b})]);

          Ciphertext multiplied_cipher;
          seal.evaluator_.multiply_plain(weight_cipher, input_plain,
                                         multiplied_cipher);
      ...
      result[get_index(shapes.conv_output[1],
                       {0, output_indexes[i][1], output_indexes[i][2],
                        output_indexes[i][3]})] = move(sum);
    } else {
      for (size_t channel = 0; channel < shapes.conv_output[1][1]; ++channel) {
        if (!is_neuron_encrypted(encrypted_neurons, round - 1, channel)) {
          for (size_t a = 0; a < shapes.kernal_sizes[round]; ++a) {
            for (size_t b = 0; b < shapes.kernal_sizes[round]; ++b) {
              auto input_values = get<vector<double>>(
                  input[get_index(shapes.conv_input[round],
                                  {0, channel, output_indexes[i][2] + a,
                                   output_indexes[i][3] + b})]);
              Plaintext input_plain;
              seal.encoder_.encode(input_values, SCALE, input_plain);

              auto weight_cipher = get<Ciphertext>(
                  weight[get_index(shapes.conv_weight[round],
                                   {output_indexes[i][1], channel, a, b})]);

              seal.evaluator_.multiply_plain_inplace(weight_cipher,
                                                     input_plain);
          ...
          for (size_t a = 0; a < shapes.kernal_sizes[round]; ++a) {
            for (size_t b = 0; b < shapes.kernal_sizes[round]; ++b) {
              auto input_cipher = get<Ciphertext>(
                  input[get_index(shapes.conv_input[round],
                                  {0, channel, output_indexes[i][2] + a,
                                   output_indexes[i][3] + b})]);
              ...
              seal.evaluator_.relinearize_inplace(weight_cipher,
                                                  seal.relin_keys_);
              seal.evaluator_.rescale_to_next_inplace(weight_cipher);
      ...
      result[get_index(shapes.conv_output[round],
                       {0, output_indexes[i][1], output_indexes[i][2],
                        output_indexes[i][3]})] = move(sum);
    }
  }
...
void thread_conv(SEALPACK &seal, const vector<array<size_t, 4>> &output_indexes,
                 const vector<variant<vector<double>, Ciphertext>> &input,
                 const vector<variant<double, Ciphertext>> &weight,
                 Ciphertext bias,
                 vector<variant<vector<double>, Ciphertext>> &result,
                 MNIST_Shape shapes, size_t round) {
  const size_t processor_count = thread::hardware_concurrency();
  vector<size_t> threads_task_count(processor_count,
                                    output_indexes.size() / processor_count);
  threads_task_count[0] += output_indexes.size() % processor_count;

  int beginnig = 0;
  vector<thread> threads(processor_count);
  for (size_t i = 0; i < processor_count; ++i) {
    threads[i] = thread(thread_conv_worker, ref(seal), cref(output_indexes),
                        cref(input), cref(weight), bias, ref(result), shapes,
                        round, beginnig, beginnig + threads_task_count[i]);
...
vector<variant<vector<double>, Ciphertext>>
conv(string dataset, SEALPACK &seal, MNIST_Shape shapes, int round,
     vector<variant<vector<double>, Ciphertext>> &input, json encrypted_neurons,
     mode work_mode) {
  if (dataset == string("MNIST")) {
    auto conv_weight =
        read_conv_weight(dataset, seal, round, encrypted_neurons);
    ...
    if (round == 1) {
      for (size_t i = 0; i < output_shape[1]; ++i) {
        if (is_neuron_encrypted(encrypted_neurons, round, i)) {
          ...
          thread_conv(seal, work_indexes, input, conv_weight, bias, output,
                      shapes, round);
          ...
          for (size_t j = 0; j < output_shape[2]; ++j) {
            for (size_t k = 0; k < output_shape[3]; ++k) {
              ...
              for (size_t channel = 0; channel < shapes.conv_output[1][1];
                   ++channel) {
                if (!is_neuron_encrypted(encrypted_neurons, round - 1,
                                         channel)) {
                  for (size_t a = 0; a < kernal_size; ++a) {
                    for (size_t b = 0; b < kernal_size; ++b) {
                      auto input_values = get<vector<double>>(
...
vector<variant<vector<double>, Ciphertext>>
avg_pool(string dataset, SEALPACK &seal, MNIST_Shape shapes, int round,
  ...
  if (dataset == string("MNIST")) {
    ...
    for (size_t i = 0; i < output_shape[1]; ++i) {
      for (size_t j = 0; j < output_shape[2]; ++j) {
        for (size_t k = 0; k < output_shape[3]; ++k) {
          if (get_if<Ciphertext>(
                  &input[get_index(input_shape, {0, i, j * 2, k * 2})]) !=
              nullptr) {
            auto cipher_one = get<Ciphertext>(
                input[get_index(input_shape, {0, i, j * 2, k * 2})]);
            ...
            for (size_t l = 0; l < batch_size; ++l) {
              ...
              pool_sum += get<vector<double>>(input[get_index(
                  input_shape, {0, i, j * 2 + 1, k * 2 + 1})])[l];
...
vector<variant<vector<double>, Ciphertext>>
recombine_input(const array<size_t, 4> input_shape, const double *input_data) {
  size_t block_size = 1;
...
void save_worker_result(string dataset,
                        vector<variant<vector<double>, Ciphertext>> &input) {
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    if (get_if<Ciphertext>(&input[i]) != nullptr) {
      ...
      for (size_t j = 0; j < values.size(); ++j) {
        result_output_stream.write(reinterpret_cast<const char *>(&values[j]),
                                   sizeof(double));
      }
    }
  }
...
void square_activate(string dataset, SEALPACK &seal,
                     vector<variant<vector<double>, Ciphertext>> &input) {
  if (dataset == string("MNIST")) {
    for (size_t i = 0; i < input.size(); ++i) {
      if (get_if<Ciphertext>(&input[i]) != nullptr) {
...
void worker(char *dataset, int batch_size, double *input_data,
            mode work_mode = separate_) {
  if (string(dataset) == string("MNIST")) {
    ...
    for (size_t round = 1; round <= shapes.conv_input.size(); ++round) {
      auto conv_result = conv(dataset, seal, shapes, round, input,
                              encrypted_neurons, work_mode);
      auto avg_pool_result =
          avg_pool(dataset, seal, shapes, round, conv_result);
