...
json get_encrypted_neurons_list(string dataset) {
  const static string NEURONS_LIST_PATH = string(PROJECT_PATH) +
                                          string("saved_models/") + dataset +
                                          ("/inference_encrypted_neurons.json");
...
bool is_neuron_encrypted(json encryped_neurons, size_t round, size_t neuron) {
  return count(encryped_neurons[to_string(round)].begin(),
               encryped_neurons[to_string(round)].end(), neuron);
}
...
SEALPACK::SEALPACK() { keygen_.create_relin_keys(relin_keys_); }
}